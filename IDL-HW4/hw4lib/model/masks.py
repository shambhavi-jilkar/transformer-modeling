...
def PadMask(padded_input, input_lengths):
    ...
    arr= torch.arange(T).expand(N,T).to(padded_input.device)
    input_lengths=input_lengths.reshape(-1,1)
    mask= arr >= input_lengths
...
def CausalMask(padded_input):
    ...
    mask= torch.triu(torch.ones((T,T),dtype=torch.bool),diagonal=1).to(padded_input.device)
    return mask # Remove once implemented
