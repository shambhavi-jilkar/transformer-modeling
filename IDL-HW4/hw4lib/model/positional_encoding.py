...
class PositionalEncoding(nn.Module):
    ...
    def create_pe_table(self, d_model, max_len):
        ...
        embeddings[:,0::2] = torch.sin(torch.outer(positions,freq))
        embeddings[:,1::2] = torch.cos(torch.outer(positions,freq))
        pe = embeddings.unsqueeze(0)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return x+encoding # Remove once implemented
