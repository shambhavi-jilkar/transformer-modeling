...
class ASRDataset(Dataset):
    def __init__(
        ...
        for i in tqdm(range(self.length)):
            ...
            feat = torch.tensor(np.load(os.path.join(self.fbank_dir, self.fbank_files[i])), dtype=torch.float32)
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feat_lengths = torch.tensor([feature.shape[0] for feature in batch_feats]) # B
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted      = [shifted[1] for shifted in batch] # B x T
            batch_golden       = [golden[2] for golden in batch] # B x T
