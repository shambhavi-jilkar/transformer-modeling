...
class SequenceGenerator:
    ...
    def generate_greedy(
        ...
        # Initialize sequences with the input
        sequences = x.clone()
        batch_size = x.size(0)
        ...
        # Generate tokens until max length or all sequences have EOS token
        for _ in range(self.max_length - x.size(1)):
            ...
            next_token_logits = self.score_fn(sequences)
            
            # Apply repetition penalty (penalize tokens that have already appeared in the sequence)
            ...
            # Convert logits to probabilities and compute log probabilities for score tracking
            ...
            # Get scores for the selected tokens
            ...
            # Fix: using self.tokenizer.eos_id instead of self.tokenizer.eos_token
