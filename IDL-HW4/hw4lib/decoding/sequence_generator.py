...
class SequenceGenerator:
    ...
    def generate_greedy(
        ...
        flags = torch.zeros(x.shape[0], dtype=torch.bool, device=x.device)
        ...
        for t in range(self.max_length-x.shape[1]):
            ...
            token_scores,_= torch.max(log_probs,dim=-1)

            scores=torch.where(flags,scores,scores+token_scores)
    ...
    def generate_beam(
        ...
        scores = torch.zeros((x.size(0), beam_width), device=self.device)
        flags = torch.zeros((x.size(0), beam_width), dtype=torch.bool, device=self.device)
        logits = self.score_fn(x)
        ...
        x=x.unsqueeze(1).expand(x.size(0), beam_width, -1)
        x = torch.cat([x, next_tokens.unsqueeze(-1)], dim=-1)
        eos= next_tokens==self.tokenizer.eos_id
        flags |= eos
        ...
        for t in range(1, self.max_length):
            ...
            for i in range(beam_width):
                logit_beam=self.score_fn(x[:,i,:])
                next_token_scores.append(logit_beam)
            ...
            cum_scores = cum_scores.view(x.shape[0],-1)
            scores, indices = cum_scores.topk(beam_width, dim=-1)
            beam_indices = indices // logits.size(-1)
            next_tokens = indices % logits.size(-1)
            flags = flags | (next_tokens == self.tokenizer.eos_id)
            x = torch.gather(x, 1, beam_indices.unsqueeze(-1).expand(-1, -1, x.size(-1)))
            x = torch.cat([x, next_tokens.unsqueeze(-1)], dim=-1)

        sorted_scores, sorted_indices = scores.sort(dim=1, descending=True)
        x = torch.gather(x, 1, sorted_indices.unsqueeze(-1).expand(-1, -1, x.size(-1)))
        return x,sorted_scores # Remove once implemented
